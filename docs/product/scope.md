...
| Reservations (creation, modification, cancellation, no-show, walk-in, waitlist, sharing, splitting, merging, multi-room, multi-guest, packages, fixed charges, traces, alerts, preferences) | T = P | Pilot executes the property's real reservation practice; group/event depth depends on OQ-017 |
...
| Rates (rate plans, BAR, derived rates, packages, negotiated rates, restrictions, effectivity, rate history) | T; P for manual rate management | Automated revenue management is D (OQ-016) |
...
| Groups & blocks (group booking, block, rooming list, pickup, master account, group billing, deposits) | T; P if group business is material | OQ-017 decides pilot depth |
...
| Sales & events (leads, opportunities, contracts, events, banquets, function space) | T; D at pilot unless conference business is material | OQ-017; conference centre suggests relevance |
| Housekeeping (room board, assignments, clean/inspect, priority rooms, discrepancies, productivity) | T = P | Required in full |
| Maintenance & engineering (work orders, assignment, priority, preventive maintenance, return to inventory) | T; P if the hotel operates it | OQ-022 decides pilot depth |
...
| POS / F&B (outlets, menus, tables, orders, room posting, outlet settlement) | T; P only if outlets operate in SuiteFlow | BR-MVP-011 defers *external POS business integration*; OQ-014 decides whether hotel outlets are operated in SuiteFlow at pilot |
| Purchasing & inventory (items, stores, suppliers, receiving, consumption, costing) | T; D at pilot except outlet/store reconciliation if outlets operate | OQ-015 |
...
| Integrations (OTAs, channel managers, booking engines, payment providers, POS, accounting, CRM, messaging, locks, ID scanners, kiosks, BI, revenue management, government systems) | T; P for the accepted payment/banking scope and any proven provider path | Other integrations depend on OQ-018/OQ-020/OQ-029 |
...
| AI assistance | T; D at pilot unless separately approved | OQ-031 |
...
| Service objective | 99.5% planning baseline, pending confirmation | BR-REL-004, OQ-009 |
...
- Statutory guest-registration requirements for identity capture (OQ-019).
...
6. Hotel outlets (restaurant/bar/conference) exist; whether they run inside SuiteFlow at pilot is OQ-014.
7. No guest-facing self-service is required at pilot.
8. Staff read English; local-language support is not required at pilot (OQ-032 is deferred, not closed).

## 7. Dependencies

- **Hotel participation**: named Operations representative and acceptance approver (OQ-003), Finance representative (OQ-002), business-day cutoff (OQ-008).
