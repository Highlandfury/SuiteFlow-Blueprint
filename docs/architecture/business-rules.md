...
| BR-FOL-005 | Comps and house use require authority, are flagged as non-revenue, and are reported daily; they never silently reduce availability accounting. Pilot comp authorisation is single-authorizer per band with maker–checker above the top band; multi-authorizer combined-limit routing is D17 depth (R2-F4). | CAP-FO-017 |
...
| BR-NAU-003 | Nightly runs (room/package charges, tax, recurring charges) are idempotent per (property, business date, run type): re-execution posts no duplicates. Conditional charge rules (conditions, amount bases, transaction-code selection) are enterprise depth (D11, R2-F3); the pilot supports fixed/recurring charges per this rule. | INV-NAU-2 |
