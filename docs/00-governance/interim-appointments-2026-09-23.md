...
depends-on: [GOV-PO-DEC, PROD-PERSONAS]
---
...
**Appointment.** The Finance Controller role-assistant persona acts as the interim Finance Controller capacity for the synthetic reference pilot. The interim human fallback is unchanged — the real Product Owner holds financial approval until a named Finance Controller is appointed (OQ-002).

**Scope.** Finance review conclusions and delegated finance direction on the financial architecture; the consolidated refund/payout/waiver table (D5); cashier variance semantics (D4); financial configuration mechanics (D6); reconciliation design; golden-day cases.
...
**Appointment.** The Technical Lead role-assistant persona acts as the interim Technical Lead capacity for the synthetic reference pilot. The fallback is unchanged — the maintaining architect provides interim design direction only; interim break-glass/incident reviewer is Technical Lead/PO (GOV-PO-DEC §B.2, D9).

**Scope.** Technical review conclusions and delegated technical direction on the target/domain/data/state architecture; ADR-001…004 and ADR-010/011 review comments; NFR, deployment and QA statements; sequencing; release and incident gates.
...
**Appointment.** The Security/Privacy Adviser role-assistant persona acts as the interim advisory security/privacy capacity for the synthetic reference pilot. Advisory-only status is unchanged (GOV-PO-DEC §B.2).

**Scope.** Security/privacy review conclusions on the security model and role matrix; ADR-009/010/011 boundaries; the incident/breach procedure (D9); exports (D10); privacy operations (D11); the AI gate (D12); the security test pack; role-played UAT and tabletop participation.
